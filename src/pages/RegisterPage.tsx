...
export default function RegisterPage() {
  return (
    <section className="mx-auto w-full max-w-2xl rounded-[32px] border border-slate-200 bg-white p-8 shadow-xl shadow-slate-200/50 dark:border-white/10 dark:bg-white/6 dark:shadow-2xl dark:shadow-black/30 dark:backdrop-blur-xl lg:p-10">
      <AuthModeToggle />
      <p className="text-[0.68rem] uppercase tracking-[0.35em] text-violet-600 dark:text-violet-200">Registro</p>
      <h1 className="mt-3 text-4xl font-semibold text-slate-900 dark:text-white">Crear cuenta</h1>
      <p className="mt-3 text-slate-600 dark:text-slate-200">Comienza con una cuenta para gestionar tu espacio.</p>

      <form className="mt-6 grid gap-4">
        <label className="grid gap-2 text-sm font-semibold text-slate-700 dark:text-slate-100">Nombre
          <input type="text" placeholder="Steven Gazo" className={inputClass} />
        </label>
        ...
        <label className="grid gap-2 text-sm font-semibold text-slate-700 dark:text-slate-100">Contraseña
          <input type="password" placeholder="Crea una contraseña" className={inputClass} />
        </label>
        <button type="submit" className="rounded-2xl bg-gradient-to-r from-violet-500 to-sky-500 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-violet-500/30 transition hover:shadow-xl hover:shadow-violet-500/40">Registrarme</button>
      </form>
    </section>
  )
}
